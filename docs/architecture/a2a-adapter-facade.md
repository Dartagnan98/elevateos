...
Add a thin HTTP facade that makes any ElevateOS agent appear as an A2A-compatible agent to external callers. Existing bus protocol stays unchanged. No agent templates need to be modified. This is a facade only — the ElevateOS bus remains the internal transport.

**Explicitly not in scope:**
- Replacing the ElevateOS bus with A2A internally
...
└────────────┬────────────┘
             ...
             ▼
┌─────────────────────────┐
│   ElevateOS Bus         │  ← existing: src/bus/
│   (file or Supabase)    │
└─────────────────────────┘
             │
             ▼
...
The facade runs as a sidecar alongside the daemon. It exposes a minimal A2A HTTP API, translates requests to ElevateOS bus messages, and polls the bus for responses.
...
Translates between A2A task states and ElevateOS bus operations:

```typescript
// Incoming A2A task → ElevateOS bus message
async function submitTask(task: A2ATaskRequest, paths: BusPaths): Promise<string> {
  ...
  // Store A2A task state indexed by ElevateOS message ID
  writeA2ATaskState(paths, task.id, { status: 'working', elevateMessageId: msgId });
  
  return task.id;
}

// Poll ElevateOS bus for response → A2A task result
async function pollForResult(taskId: string, paths: BusPaths): Promise<A2ATask> {
  const state = readA2ATaskState(paths, taskId);
  const response = findResponseToMessage(paths, state.elevateMessageId);
...
Submit a new task. Translates to a ElevateOS inbox message sent to the agent.
...
- `POST /tasks/send` → ElevateOS bus message
...
1. **Multi-agent discovery:** If all ElevateOS agents on a machine run A2A servers, should there be a registry endpoint (e.g. `GET /agents`) listing the fleet? Useful for orchestrators from other frameworks.

2. **Task timeout:** If the ElevateOS agent takes > N seconds to reply (context window, complex task), A2A clients may time out. What is the right timeout, and should we implement long-polling or SSE streaming to handle slow responses?
...
4. **Outbound A2A (client):** v1 is server-only. When should we add A2A client capability so ElevateOS agents can call other A2A agents as tools?
