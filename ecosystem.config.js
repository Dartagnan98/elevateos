// PM2 ecosystem config for ElevateOS daemon.
...
const FRAMEWORK_ROOT = process.env.ELEVATE_FRAMEWORK_ROOT || process.env.CTX_FRAMEWORK_ROOT || __dirname;
const PROJECT_ROOT = process.env.ELEVATE_PROJECT_ROOT || process.env.CTX_PROJECT_ROOT || FRAMEWORK_ROOT;
const INSTANCE_ID = process.env.ELEVATE_INSTANCE_ID || process.env.CTX_INSTANCE_ID || 'default';
const CTX_ROOT = process.env.ELEVATE_ROOT || process.env.CTX_ROOT || path.join(os.homedir(), '.elevate', INSTANCE_ID);
const CTX_ORG = process.env.ELEVATE_ORG || process.env.CTX_ORG || '';
  ...
  apps: [
    {
      ...
      cwd: FRAMEWORK_ROOT,
      env: {
        ELEVATE_INSTANCE_ID: INSTANCE_ID,
        ELEVATE_ROOT: CTX_ROOT,
        ELEVATE_FRAMEWORK_ROOT: FRAMEWORK_ROOT,
        ELEVATE_PROJECT_ROOT: PROJECT_ROOT,
        ELEVATE_ORG: CTX_ORG,
        ...
        // paths during development. `kill -SIGUSR2 $(pm2 pid elevate-daemon)`
      ...
      // `pm2 restart elevate-daemon`. That is intentional: storm
