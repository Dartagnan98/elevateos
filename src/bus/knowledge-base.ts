...
function buildKBEnv(
  ...
  // (~/.elevate/<instance>/orgs/<lowercase>/knowledge-base/) with its own
  ...
  const kbRoot = join(getStateRoot(instanceId), 'orgs', canonicalOrg, 'knowledge-base');
...
export function ingestKnowledgeBase(
  ...
  const kbRoot = join(getStateRoot(instanceId), 'orgs', org, 'knowledge-base');
