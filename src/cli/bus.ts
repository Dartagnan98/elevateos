...
busCommand
  ...
  .action(async (opts: { reason?: string }) => {
    ...
    const ctxRoot = require('path').join(require('os').homedir(), '.elevate', env.instanceId);
...
busCommand
  ...
  .action((opts: { org?: string }) => {
    ...
    const kbRoot = pjoin(hdir(), '.elevate', instanceId, 'orgs', org, 'knowledge-base');
...
busCommand
  ...
  .action((opts: { format?: string; allOrgs?: boolean }) => {
    ...
    if (opts.allOrgs) {
      ...
      const ctxRoot = join(homedir(), '.elevate', env.instanceId);
...
busCommand
  ...
  .action((opts: { dryRun?: boolean }) => {
    ...
    if (!opts.dryRun && patched > 0) {
      ...
      console.log('  elevate restart <agent-name>');
    }
  });
