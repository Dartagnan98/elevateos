...
import { CLI_NAME, getFrameworkRoot, getStateRoot } from '../utils/elevate.js';

/**
 * Discover the Elevate framework root without depending on
 * `process.cwd()`. Order of precedence:
 *   1. ELEVATE_FRAMEWORK_ROOT / CTX_FRAMEWORK_ROOT env var
 *   2. CTX_PROJECT_ROOT env var (legacy alias)
 *   3. ~/elevateos/ (the canonical install location)
...
function getEnabledAgentsPath(instanceId: string): string {
  return join(getStateRoot(instanceId), 'config', 'enabled-agents.json');
}
...
function writeEnabledAgents(instanceId: string, agents: Record<string, any>): void {
  const path = getEnabledAgentsPath(instanceId);
  const dir = join(getStateRoot(instanceId), 'config');
...
export const enableAgentCommand = new Command('enable')
  ...
  .action(async (agent: string, options: { instance: string; org?: string }) => {
    ...
    if (!agentEnvPath) {
      ...
      console.error(`(Set ELEVATE_FRAMEWORK_ROOT to override path discovery, or run from inside ~/elevateos.)`);
    ...
    try {
      ...
      if (validation.ok) {
        ...
        console.error(`  ${formatValidateError(validation)}`);
        console.error(`  Edit ${agentEnvPath} and re-run: ${CLI_NAME} enable ${agent}`);
        process.exit(1);
      }
    } catch (err) {
    ...
    if (running) {
      const response = await ipc.send({ type: 'start-agent', agent, source: `${CLI_NAME} enable` });
      if (response.success) {
        console.log(`  Started via daemon: ${response.data}`);
      }
    } else {
      console.log(`  Daemon not running. Start with: ${CLI_NAME} start`);
    }
  });
...
export const disableAgentCommand = new Command('disable')
  ...
  .action(async (agent: string, options: { instance: string }) => {
    ...
    if (running) {
      ...
      // Pattern matches src/cli/bus.ts:1285-1289.
      writeDisableMarker(options.instance, agent, `disabled via ${CLI_NAME} disable`);

      const response = await ipc.send({ type: 'stop-agent', agent, source: `${CLI_NAME} disable` });
      if (response.success) {
