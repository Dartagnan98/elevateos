...
export const uninstallCommand = new Command('uninstall')
  ...
  .description('Remove ElevateOS state directories and PM2 processes')
  .action(async (options: { instance: string; force?: boolean; keepState?: boolean }) => {
    const instanceId = options.instance;
    const ctxRoot = join(homedir(), '.elevate', instanceId);

    if (!existsSync(ctxRoot)) {
      console.log(`No ElevateOS state found at ${ctxRoot}`);
      return;
    }

    console.log(`\nUninstalling ElevateOS instance: ${instanceId}`);
    ...
    try {
      ...
      if (pm2Result.status === 0 && pm2Result.stdout) {
        ...
        const elevateProcesses = processes.filter((p: { name: string }) =>
          p.name.startsWith('elevate-') || p.name.startsWith(`ctx-${instanceId}`),
        );
        for (const p of elevateProcesses) {
