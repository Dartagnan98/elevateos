...
**Slash commands from Telegram:** If the message text starts with `/` (e.g. `/loop 4h heartbeat`, `/commit`, `/restart`), treat it as a command to execute — use the Skill tool to invoke it. Parse: `/skillname [args]` → invoke `Skill(skill="skillname", args="[args]")`. Common commands: `/loop` (create cron), `/compact` (compact context), `/restart` (restart agent). If the skill doesn't exist, reply telling the user which skills are available via `elevateos bus list-skills`.
