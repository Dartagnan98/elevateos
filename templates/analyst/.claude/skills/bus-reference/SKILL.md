...
```bash
elevateos bus log-event <category> <event_name> <severity> '[json_payload]'
```
...
elevateos bus log-event work research_complete info --meta '{"topic":"competitor analysis","findings":3}'
```
...
Example:
```bash
elevateos bus create-approval "Send cold outreach to 50 leads" comms "Draft email attached in task_abc123. Target list: SaaS founders."
```
