...
> The definitive guide for managing ElevateOS agent lifecycle. Every operation, every script, every protocol. Follow these EXACTLY - do not improvise.
...
> "Do you have an existing agent, workspace, or information that you want to migrate or seed into this ElevateOS agent before ElevateOS onboarding? Supports migrating any type of agent workspace or other information type."
...
# 4. Choose a username (e.g., myagent_elevate_bot)
...
# Step 5: Write .env (CRITICAL - do this BEFORE elevate enable)
...
const c = JSON.parse(fs.readFileSync(path));
if (!c.crons) c.crons = [];
c.crons.push({ name: 'new-cron', interval: '1h', prompt: 'Do the thing' });
fs.writeFileSync(path, JSON.stringify(c, null, 2));
"
...
### Enable
```bash
elevate enable <agent> --org <org>
```

### Disable
```bash
elevate disable <agent> --org <org>
```
...
```bash
cat "$HOME/.elevate/default/state/$AGENT/heartbeat.json"
```
...
### Reset Crash Counter
```bash
rm -f "$HOME/.elevate/default/state/$AGENT/.crash_count_today"
```

### Force Fresh Start (Lose Conversation)
```bash
echo "" > "$HOME/.elevate/default/state/$AGENT/.force-fresh"
elevate enable "$AGENT" --org "$ORG" --restart
```
...
1. Check crash count: `cat $HOME/.elevate/default/state/$AGENT/.crash_count_today`
2. Check stderr: `tail -20 $HOME/.elevate/default/logs/$AGENT/stderr.log`
3. Common causes: rate limit, auth expired, context exhaustion
4. Fix: reset crash count, fix root cause, `elevate enable <agent> --restart`
