...
Then add the safe recurring entry to `config.json` and create the live cron with CronCreate:

Use only safe intervals (`10m`, `30m`, `1h`, `6h`, `24h`). If the requested loop interval is not in that set, choose the nearest safe interval and gate inside the prompt.

Then add to `config.json` crons array:
```json
{"name": "experiment-<metric>", "type": "recurring", "interval": "24h", "prompt": "Read .claude/skills/autoresearch/SKILL.md and execute the experiment loop. If the requested experiment cadence is longer than 24h, check the last run timestamp first and exit early until enough time has elapsed."}
```
