...
1. **You** (the ElevateOS agent) act as the human/supervisor
...
export CTX_ROOT="$HOME/.elevate/default"
```
...
- Your domain knowledge as a ElevateOS agent
...
- What skills could help the worker? (existing ElevateOS skills)
...
Copy relevant ElevateOS skills to the worker's project:
```bash
# If the worker needs browser automation knowledge, install via community catalog:
elevateos bus install-community-item playwright-automation
```
...
Tell the worker to use the safe cron path:
```bash
elevateos bus send-message <worker-name> normal \
  'Add a 10m recurring entry to config.json, run CronList, and create the live cron with CronCreate to check START.md for pending tasks. If not working on a task, pick the next one.'
```
