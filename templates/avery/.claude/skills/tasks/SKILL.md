...
### 4. Complete
```bash
elevateos bus complete-task <task_id> --result "[output summary]"
```

### 5. Log KPI (if measurable)
```bash
elevateos bus log-event task task_completed info --meta '{"task_id":"ID","kpi_key":"metric_name","value":1}'
```
...
| Action | Command |
|--------|---------|
| Create | `elevateos bus create-task "<title>" --desc "<desc>" [--assignee <a>] [--priority <p>] [--run-at ISO] [--due-date ISO]` |
| List | `elevateos bus list-tasks [--status S] [--agent A] [--priority P]` |
| Update | `elevateos bus update-task <id> <status>` |
| Complete | `elevateos bus complete-task <id> --result "[summary]"` |
| Log event | `elevateos bus log-event <category> <event> <severity> --meta '[json]'` |
