...
> **Environment variables**: `CTX_ROOT`, `CTX_FRAMEWORK_ROOT`, `CTX_ORG`, `CTX_AGENT_NAME`, `CTX_TELEGRAM_CHAT_ID`, and `CTX_INSTANCE_ID` are automatically set by the ElevateOS framework.

> **ElevateOS Tier 1 cron rule:** the installer writes the safe interval-only crons from `config.json` before first boot. Do not create new crons during onboarding. If a legacy section below mentions `/loop`, time-anchored cron expressions, or unsupported intervals, skip that instruction and only verify the existing config.
...
The orchestrator has built-in crons from `config.json`. Verify them before normal operations.

### Step 11: Verify core crons

Run CronList first (avoid duplicates), then compare against `config.json`.

**Safe interval policy**

Use only `10m`, `30m`, `1h`, `6h`, and `24h` from `config.json`. Missing crons should be restored with CronCreate directly, not `/loop`.

Verify config uses safe intervals:

```bash
jq -e 'all(.crons[]; [.interval] | inside(["10m","30m","1h","6h","24h"]))' config.json
```
...
> "Are you setting this system up from scratch, or migrating from an existing ElevateOS instance or another workspace?
>
...
- (g) Desired cadence - how often the experiment loop should run (often same as window)
- (h) Approval required before running each experiment?

Then follow `.claude/skills/autoresearch/SKILL.md` setup with a safe interval. Do not create an unsupported or time-anchored cron:
```
Record the desired experiment cadence in MEMORY.md and ask Avery to schedule it after safe-interval review.
```
