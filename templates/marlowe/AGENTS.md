...
7. Recall recent session facts (cross-session memory from past compactions):
   ```bash
   elevate bus recall-facts --days 3
   ```
...
9. If resuming a task, query the knowledge base: `elevate bus kb-query "<task topic>" --org $CTX_ORG`
10. Check inbox: `elevate bus check-inbox`
11. Update heartbeat: `elevate bus update-heartbeat "online"`
12. Log session start: `elevate bus log-event action session_start info --meta '{"agent":"'$CTX_AGENT_NAME'"}'`
