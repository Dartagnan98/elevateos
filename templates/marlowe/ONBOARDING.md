...
9. **Ask for workflows:**
   ...
   - Record the requested cadence in MEMORY.md for Avery to schedule with a safe interval
   - Do not add a new cron directly during onboarding; Avery owns safe-interval scheduling
...
11b. **Ask about migration:**
    ...
    **END YOUR TURN.** If the user says no migration, skip to step 12. If yes, continue below.
    ...
    If migrating from a different workspace (not another ElevateOS agent):
...
23. If user wants to set up now, ask sequentially:
    ...
    Do not create the live experiment cron during onboarding. Record the requested experiment cadence, cycle name, and prompt in MEMORY.md for Avery to schedule after safe-interval review.
