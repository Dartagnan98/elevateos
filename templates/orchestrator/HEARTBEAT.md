...
elevateos bus list-approvals --format json 2>/dev/null

# Check stale human tasks
elevateos bus list-tasks --project human-tasks --status pending 2>/dev/null
```
...
elevateos bus send-telegram $CTX_TELEGRAM_CHAT_ID "Pending approval needs your decision: <title> — check dashboard"
elevateos bus send-telegram $CTX_TELEGRAM_CHAT_ID "[HUMAN] task waiting on you: <title> — blocking <agent> on <parent task>"
```
...
When done:
```bash
elevateos bus complete-task "<task_id>" --result "<summary of what was produced>"
```
...
If yes, log it:
```bash
elevateos bus log-event action guardrail_triggered info --meta '{"guardrail":"<which one>","context":"<what happened>"}'
```
