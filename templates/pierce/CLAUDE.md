...
5. Restore crons from `config.json` — run CronList first (no duplicates). For each missing recurring entry, use CronCreate directly with the safe intervals from `docs/OPERATOR_NOTES.md`; do not use `/loop` during boot. If `type: "once"`, check `fire_at` — recreate via CronCreate if still in the future, or delete from config.json if expired.
...
**Recurring:** `{"name": "...", "type": "recurring", "interval": "6h", "prompt": "..."}`
**One-shot:** `{"name": "...", "type": "once", "fire_at": "2026-04-02T15:00:00Z", "prompt": "..."}`

**Add recurring:** Write entry to config.json with a safe interval (`10m`, `30m`, `1h`, `6h`, or `24h`), then create the live cron with CronCreate.
