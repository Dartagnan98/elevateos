...
> **Environment variables**: `CTX_ROOT`, `CTX_FRAMEWORK_ROOT`, `CTX_ORG`, `CTX_AGENT_NAME`, and `CTX_INSTANCE_ID` are automatically set by the ElevateOS framework. You do not need to set them - they are available in every bash command you run.
...
> **ElevateOS Tier 1 cron rule:** the installer writes the safe interval-only crons from `config.json` before first boot. Do not create new crons during onboarding. If a legacy section below mentions `/loop`, time-anchored cron expressions, or unsupported intervals, skip that instruction and only verify or record the intended workflow.
...
# Keep config.json crons[0].interval in the safe set: 10m, 30m, 1h, 6h, or 24h
```
...
**Always verify this interval-based cron from config.json:**

- Heartbeat (every 6h): `Read HEARTBEAT.md and follow its instructions. Update your heartbeat, check inbox, and work on your highest priority task.`

Run CronList, then verify `config.json` already has a `heartbeat` entry (it does by default - skip adding a duplicate).
...
> "I have a heartbeat cycle every 6 hours and daily metrics collection. Want me to note any other recurring checks for Avery to schedule safely? For example: daily reports, integration health checks, custom monitoring."

For each additional cron the user requests:
- Record the requested interval and prompt in MEMORY.md for safe scheduling
...
> 2. Framework updates - I check for ElevateOS updates and tell you what changed before applying
...
For each enabled feature, only add a safe interval entry to config.json after user approval:

**local_version_control** - safe interval option:
```
interval: "24h"
...
Add to config.json:
```json
{"name": "auto-commit", "type": "recurring", "interval": "24h", "prompt": "Run TZ=America/Vancouver date +%H. If hour not in {21,22,23}, exit silently. Otherwise: run daily git snapshot. elevateos bus auto-commit - review the staged diff for PII - commit with descriptive message. Never push."}
```

**upstream_sync** - safe interval option:
```
interval: "24h"
...
Add to config.json:
```json
{"name": "check-upstream", "type": "recurring", "interval": "24h", "prompt": "Run TZ=America/Vancouver date +%H. If hour not in {21,22,23}, exit silently. Otherwise: check for framework updates: elevateos bus check-upstream. If updates available, explain every change in plain English via Telegram and wait for explicit approval before applying. Never apply during night mode."}
```

**catalog_browse** - safe interval option:
```
interval: "24h"
...
Add to config.json:
```json
{"name": "catalog-browse", "type": "recurring", "interval": "24h", "prompt": "Run TZ=America/Vancouver date +%A. If output is not Sunday, exit silently. Otherwise: browse community catalog: elevateos bus browse-catalog. Surface ONE relevant new item to user via Telegram. If they say install it: elevateos bus install-community-item <name>. If they decline, skip that item for 30 days."}
```
...
Add to config.json:
```json
{"name": "theta-wave", "type": "recurring", "interval": "24h", "prompt": "Run TZ=America/Vancouver date +%H. If hour not in {21,22,23}, exit silently. Otherwise: read .claude/skills/theta-wave/SKILL.md. Initiate the theta wave cycle. First action: message the orchestrator that theta wave is starting and share your initial system scan."}
```
...
> "Your ElevateOS system is all set up and ready to work.
>
