...
beforeEach(() => {
  testDir = mkdtempSync(join(tmpdir(), 'elevate-approval-test-'));
  ...
  // (~/ElevateOS/ vs ~/.elevate/default/) and the approval bug shipped
  // because the original code conflated them.
  frameworkRoot = mkdtempSync(join(tmpdir(), 'elevate-framework-test-'));
